...
function computeGap(it, uid, opts) {
  ...
  return { gapScore: Math.min(100, score), gapReasons: reasons.slice(0, 4), deadlineTier: deadlineTier(days) };
}

function withGap(it, uid, opts) {
  const g = computeGap(it, uid, opts);
  return Object.assign({}, it, { gapScore: g.gapScore, gapReasons: g.gapReasons, deadlineTier: g.deadlineTier });
}
