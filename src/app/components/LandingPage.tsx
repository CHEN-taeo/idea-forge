  ...
  return (
    <div className="if-page px-4 pb-8 max-w-md mx-auto">
      <div className="text-center pt-10 pb-6">
        <div className="if-mark" aria-hidden>
          <BrandMark size={28} />
        </div>
        <h1 className="font-display text-[1.65rem] text-[var(--if-ink)] tracking-tight">
          {brand.productName}
        </h1>
        <p className="text-xs text-[var(--if-muted)] mt-1 tracking-wide">
          {brand.subtitle}
        </p>
      ...
      <p className="text-[10px] text-center text-[var(--if-muted-soft)] uppercase tracking-[0.12em] mb-3">
        主路径 · 拾念 → 论道 → 落契
      </p>
      ...
      <div className="mt-1 space-y-0">
        ...
        <button
          ...
          disabled={!connected}
          className={`if-mode-tile ${connected ? '' : 'opacity-60'}`}
        >
          <span className="if-mode-icon">🔥</span>
          <span className="flex-1 min-w-0 text-left">
            <span className="block text-[0.95rem] font-medium text-[var(--if-ink)]">
